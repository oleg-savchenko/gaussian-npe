...
class UNet(nn.Module):
    def __init__(
        ...
        out_classes: int = 2,
        dimensions: int = 3,
        num_encoding_blocks: int = 4,
        out_channels_first_layer: int = 24,
        condition_dim=None,
    ...
    def forward(self, x, cond=None):
        skip_connections, encoding = self.encoder(x, cond)
        encoding = self.bottom_block(encoding, cond)
        x = self.decoder(skip_connections, encoding, cond)
...
class Encoder(nn.Module):
    ...
    def forward(self, x, cond=None):
        ...
        for encoding_block in self.encoding_blocks:
            x, skip_connnection = encoding_block(x, cond)
...
class EncodingBlock(nn.Module):
    def __init__(
        ...
        if condition_dim is not None:
            self.cond_proj = zero_init(
                nn.Linear(condition_dim, out_channels_first, bias=False)
            )
    ...
    def forward(self, x, cond=None):
        if self.residual:
            ...
            x = self.conv1(x)
            if cond is not None:
                x = x + self.cond_proj(cond)[:, :, None, None, None]
            x = self.conv2(x)
...
class Decoder(nn.Module):
    ...
    def forward(self, skip_connections, x, cond=None):
        zipped = zip(reversed(skip_connections), self.decoding_blocks)
        for skip_connection, decoding_block in zipped:
            x = decoding_block(skip_connection, x, cond)
        return x
...
class DecodingBlock(nn.Module):
    def __init__(
        ...
        if condition_dim is not None:
            self.cond_proj = zero_init(
                nn.Linear(condition_dim, out_channels, bias=False)
            )

    def forward(self, skip_connection, x, cond=None):
        x = self.upsample(x)
